...
1. Добавлена поддержка x64.
2. Теперь для сборки РефалАБ поддерживается только компилятор Clang.
Компилятор gcc больше не поддерживается.
3. Внесены изменения в файл заголовков include\refal.def для разработчиков.
4. Внесены небольшие изменения в работе компилятора и интерпретатора.
