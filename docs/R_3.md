...
имеются интерфейсные функции transplantation и find_duplicate.
...
заключенный между begin и after, в метакоде-Б.
...
#### Функция get_string_argument

НАЗНАЧЕНИЕ:

Формирует строку из выражения символов-литер аргумента первичной функции.

ОБЪЯВЛЕНИЕ: extern T_LINKCB *get_string_argument(char *string, size_t max_string_size, T_LINKCB *begin);

ОБРАЩЕНИЕ: get_string_argument(string, max_string_size, begin);
...
Возвращаемое значение - указатель на звено -
символ после последнего символа-литера строки string.

ИСПОЛЬЗОВАНИЕ:

Используется, когда необходимо в функции на Си
сформировать строку string с нуль-символом '\0' в конце
из выражения символов-литер аргумента функции, начиная со звена begin.
Длина строки string ограничена max_string_size. Вырабатывает значение
