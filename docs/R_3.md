...
Поле tag содержит признак типа звена. Если его двадцать девять старших разрядов нулевые,
звено называется стандартным, если же хотя бы один из двадцати девяти
...
нестандартные звенья, у которых старшие двадцать девять разрядов
...
ящика;
- для символа нестандартного типа поле info содержит произвольную
(с точки зрения РефалАБ-машины) информацию.
...
Обращение к функции должно иметь вид:
    ...
    change_plus_minus(before, after);

где before - указатель на звено, предшествующее выражению,
а after - указатель на звено, следующее за выражением.

Функция может быть описана следующим образом:

    void change_plus_minus(const T_LINKCB *before, const T_LINKCB *after)
    {
        T_LINKCB *current = before->next;
        while (current != after)
        {
            if (current->tag == TAGO && current->info.infoc == '+')
                current->info.infoc = '-';
...
символы-литеры '(' и ')' образуют правильную скобочную структуру.
    ...
    void brackets_match(const T_LINKCB *before, const T_LINKCB *after)
    {
        ...
        while (current != after)
        {
            if (current->tag == TAGO && current->info.infoc == '(')
            {
                current->info.codep = last_bracket;
                last_bracket = current;
            }
            else if (current->tag == TAGO && current->info.infoc == ')')
            {
                current->info.codep = last_bracket;
                current->tag = TAGRB;
                T_LINKCB *temp_last_bracket = last_bracket->info.codep;
                last_bracket->info.codep = current;
...
Для работы с выражениями имеются несколько интерфейсных функций.
...
Далее в звене, на которое указывает linkcb, формируется символ-метка FEOF,
...
#### Функция compare_expressions_lexicographic

НАЗНАЧЕНИЕ:

Сравнивает два выражения лексикографически.

ОБЪЯВЛЕНИЕ: extern int8_t compare_expressions_lexicographic(const T_LINKCB *before, const T_LINKCB *middle, const T_LINKCB *after);

ОБРАЩЕНИЕ: compare_expressions_lexicographic(before, middle, after);

ПАРАМЕТРЫ:
- before - указатель на звено, предшествующее первому выражению;
- middle - указатель на звено, находящееся между двумя выражениями;
- after - указатель на звено, следующее за вторым выражением.

Возвращаемое значение - результат сравнения от -1 до 1.

ИСПОЛЬЗОВАНИЕ:

Сравнивает два выражения лексикографически. Первое выражение находится между before и middle.
Второе выражение находится между middle и after.
Если первое выражение больше второго, функция вырабатывает значение 1.
Если первое выражение меньше второго, функция вырабатывает значение -1.
Если оба выражения равны, функция вырабатывает значение 0.
...
### 3.5. Работа с целыми числами

Целые числа ([п.2.2](R_2.md#22-%D1%84%D1%83%D0%BD%D0%BA%D1%86%D0%B8%D0%B8-%D0%B8-%D1%81%D0%BF%D0%B5%D1%86%D0%B8%D1%84%D0%B8%D0%BA%D0%B0%D1%82%D0%BE%D1%80%D1%8B-%D0%B0%D1%80%D0%B8%D1%84%D0%BC%D0%B5%D1%82%D0%B8%D0%BA%D0%B8)) в выражении
представляются в виде цепочки звеньев: макроцифр, перед которыми
может находится знак числа - символ-литера '+' или '-'.
Целое число нуль может быть представлено в виде пустого выражения.

Структура целого числа описывается следующим образом:
...
Отдельные поля целого числа имеют следующее содержимое:

- .sign - знак целого числа. Если число положительное, то sign = '+'.
Если число отрицательное, то sign = '-'. Если число - нуль, то sign либо
'+', либо '-'.
- .begin - указатель на первое звено цепочки макроцифр целого числа.
Если цепочка макроцифр числа начинается с ведущих нулей, то begin
указывает на первое ненулевое звено. Если число - нуль, то begin
не определено.
- .end - указатель на последнее звено цепочки макроцифр целого числа.
Если число - нуль, то end не определено.
- .length - количество макроцифр целого числа. Не учитываются
нулевые макроцифры, с которых начинается цепочка макроцифр числа.
Если число - нуль, то length = 0. 

Для работы с целыми числами имеются несколько интерфейсных функций.

#### Функция read_big_number_expression

НАЗНАЧЕНИЕ:

Читает целое число из выражения.

ОБЪЯВЛЕНИЕ: extern bool read_big_number_expression(T_BIG_NUMBER *big_number, const T_LINKCB *before, const T_LINKCB *after);

ОБРАЩЕНИЕ: read_big_number_expression(big_number, before, after);

ПАРАМЕТРЫ:
- big_number - указатель на целое число из выражения;
- before - указатель на звено, предшествующее целому числу из выражения;
- after - указатель на звено, следующее за целым числом из выражения.

ИСПОЛЬЗОВАНИЕ:

Пытается прочитать целое число из выражения big_number, находящееся между before и after.
Если в выражении находится целое число, функция вырабатывает значение true.
Иначе функция вырабатывает значение false.
...
#### Функция read_big_numbers_expression

НАЗНАЧЕНИЕ:

Читает два целых числа из выражения.

ОБЪЯВЛЕНИЕ: extern bool read_big_numbers_expression(T_BIG_NUMBER *big_number1, T_BIG_NUMBER *big_number2, const T_LINKCB *before, const T_LINKCB *middle, const T_LINKCB *after);

ОБРАЩЕНИЕ: read_big_numbers_expression(big_number1, big_number2, before, middle, after);

ПАРАМЕТРЫ:
- big_number1 - указатель на первое целое число из выражения;
- big_number2 - указатель на второе целое число из выражения;
- before - указатель на звено, предшествующее первому целому числу из выражения;
- middle - указатель на звено, находящееся между двумя целыми числами из выражения;
- after - указатель на звено, следующее за вторым целым числом из выражения.

ИСПОЛЬЗОВАНИЕ:

Пытается прочитать первое целое число из выражения big_number1, находящееся между before и middle,
а потом прочитать второе целое число из выражения big_number2, находящееся между middle и after.
Если в выражении находятся два целых числа, функция вырабатывает значение true.
Иначе функция вырабатывает значение false.
...
#### Функция exchange_big_numbers

НАЗНАЧЕНИЕ:

Меняет местами два целых числа.

ОБЪЯВЛЕНИЕ: extern void exchange_big_numbers(T_BIG_NUMBER *big_number1, T_BIG_NUMBER *big_number2);

ОБРАЩЕНИЕ: exchange_big_numbers(big_number1, big_number2);

ПАРАМЕТРЫ:
- big_number1 - указатель на первое целое число;
- big_number2 - указатель на второе целое число.

ИСПОЛЬЗОВАНИЕ:

Меняет местами два целых числа big_number1 и big_number2.
...
#### Функция compare_big_numbers_absolute

НАЗНАЧЕНИЕ:

Сравнивает два целых числа по модулю.

ОБЪЯВЛЕНИЕ: extern int8_t compare_big_numbers_absolute(const T_BIG_NUMBER *big_number1, const T_BIG_NUMBER *big_number2);

ОБРАЩЕНИЕ: compare_big_numbers_absolute(big_number1, big_number2);
...
Сравнивает два целых числа big_number1 и big_number2 по модулю.
Если первое число больше второго, функция вырабатывает значение 1.
Если первое число меньше второго, функция вырабатывает значение -1.
Если оба числа равны, функция вырабатывает значение 0.
...
#### Функция compare_big_numbers

НАЗНАЧЕНИЕ:

Сравнивает два целых числа.

ОБЪЯВЛЕНИЕ: extern int8_t compare_big_numbers(const T_BIG_NUMBER *big_number1, const T_BIG_NUMBER *big_number2);

ОБРАЩЕНИЕ: compare_big_numbers(big_number1, big_number2);
...
Сравнивает два целых числа big_number1 и big_number2.
Если первое число больше второго, функция вырабатывает значение 1.
Если первое число меньше второго, функция вырабатывает значение -1.
Если оба числа равны, функция вырабатывает значение 0.
...
### 3.6. Представление функциональных скобок в списковой памяти
...
### 3.7. Представление ящиков в списковой памяти
...
содержит константу 0x4E, что дает возможность проверить, является ли
...
- .free_memory_head - адрес звена, являющегося головой списка свободных звеньев.
...
### 3.11. Инициализация РефалАБ-машины
...
### 3.12. Пространство списковой памяти и список свободных звеньев
...
блоке refal в поле free_memory_head содержится ссылка на звено - голову этого
списка.
...
#### Функция check_count_free_memory
...
ОБЪЯВЛЕНИЕ: extern bool check_count_free_memory(size_t count);

ОБРАЩЕНИЕ: check_count_free_memory(count);
...
ОБЪЯВЛЕНИЕ: extern void insert_to_free_memory(T_LINKCB *before, T_LINKCB *after);

ОБРАЩЕНИЕ: insert_to_free_memory(before, after);
...
#### Функция insert_from_free_memory
...
ОБЪЯВЛЕНИЕ: extern bool insert_from_free_memory(T_LINKCB *where, size_t count);

ОБРАЩЕНИЕ: insert_from_free_memory(where, count);
...
на которое указывает where. В этом случае значением insert_from_free_memory является true,
...
ЗАМЕЧАНИЯ:
1) в результате работы insert_from_free_memory ни where, ни count не меняются;
2) если count = 0, insert_from_free_memory ничего не делает и вырабатывает значение true;
...
### 3.13. Таблица состояния процесса
...
указывающий максимальный предельный номер шага:
...
#### Функция print_status_table

НАЗНАЧЕНИЕ:

Печатает количество шагов, поле зрения
и копилку в метакоде, и, если надо, размер списковой памяти
в звеньях и байтах, время работы интерпретатора языка сборки.

ОБЪЯВЛЕНИЕ: extern void print_status_table(T_STATUS_TABLE *status_table, bool additional_info);

ОБРАЩЕНИЕ: print_status_table(status_table, additional_info);

ПАРАМЕТРЫ:
- status_table - таблица состояния процесса;
- additional_info - признак печати дополнительной информации.

ИСПОЛЬЗОВАНИЕ:

Используется для печати количества шагов, поля зрения и копилки
в метакоде процесса, имеющего таблицу состояния status_table,
и, если additional_info = true, размера списковой памяти
в звеньях и байтах, времени работы интерпретатора языка сборки.

### 3.14. Создание и уничтожение процессов
...
ПАРАМЕТРЫ: status_table - таблица состояния процесса.
...
ПАРАМЕТРЫ:
- status_table - таблица состояния процесса;
- refalab_function - имя функции Function с маленькими латинскими буквами и с префиксом "refalab_".
...
"<Function ", а после содержимого поля зрения - ">". Таким образом, если поле
зрения содержало выражение e.X, оно приобретает вид "\<Function e.X>". После этого
...
### 3.16. Примеры управления процессами

**Пример 1**

Следующая программа создает процесс, запускает его, а затем печатает
причину его остановки. Затем печатаются количество шагов,
поле зрения и копилка в метакоде, размер списковой памяти
в звеньях и байтах, время работы интерпретатора языка сборки и процесс уничтожается.
...
### 3.18. Динамический захват списковой памяти

Алгоритм управления списковой памятью работает следующим образом.

В самом начале работы РефалАБ-программы, если максимальное количество звеньев
меньше минимального количества звеньев, алгоритм считает, что доступная память
исчерпана, ибо ее не хватает даже на создание начальной списковой памяти
размером в минимальное количество звеньев. Если же максимальное количество звеньев
больше или равно минимального количества звеньев, под список захватывается 
минимальное количество звеньев.

Во время дальнейшей работы, алгоритм постоянно помнит текущий размер
списковой памяти в звеньях. Всякий раз, когда возникает нехватка
списковой памяти, делается попытка собрать мусор. Если в результате
успешной сборки мусора в списке свободных звеньев не менее,
чем заданное количество звеньев, считается, что памяти под список
достаточно и работа продолжается. Если же сборка мусора завершилась неуспешно или
в списке свободных звеньев менее, чем заданное количество  звеньев,
алгоритм пытается захватить дополнительную память. Если текущий размер
списковой памяти равен максимальному размеру списковой памяти,
то список уже достиг максимального размера и алгоритм объявляет, что
свободная память исчерпана. В противном случае, к списку добавляются
дополнительные звенья, но не больше чем разница между максимальным размером
списковой памяти и текущим размером списковой памяти, и работа продолжается.
...
#### Функция extended_insert_from_free_memory
...
ОБЪЯВЛЕНИЕ: extern bool extended_insert_from_free_memory(T_LINKCB *where, size_t count);

ОБРАЩЕНИЕ: extended_insert_from_free_memory(where, count);
...
на которое указывает where в результате обращения к функции insert_from_free_memory
...
### 3.19. Вызов программы на Си из РефалАБ-программы
...
### 3.20. Написание первичных функций на Си
...
указываемому в РефалАБ-программе в директиве EXTERN, с добавлением
...
Макрос mdebug вместе с директивой #if defined используются для кода,
используемого в режиме отладки.

    #if defined mdebug
    <код в режиме отладки>
    #endif

### 3.21. Примеры первичных функций на Си
...
Описание на Си имеет вид:
    ...
    static void cpfm_(void)
    {
        ...
        T_LINKCB *current_result = refal.previous_result->next;
        while (current_result != refal.next_result)
        {
            if (current_result->tag == TAGO && current_result->info.infoc == '+')
                current_result->info.infoc = '-';
...
и s.Z='>', если код o.X больше кода o.Y.
    ...
    static void crel_(void)
    {
        do
        {
            const T_LINKCB *first_symbol_char = refal.previous_argument->next;
            if (first_symbol_char->tag != TAGO)
                break;
            const T_LINKCB *second_symbol_char = first_symbol_char->next;
            if (second_symbol_char->next != refal.next_argument || second_symbol_char->tag != TAGO)
                break;
            char symbol_result = '=';
            if ((uint8_t)first_symbol_char->info.infoc > (uint8_t)second_symbol_char->info.infoc)
                symbol_result = '>';
            else if ((uint8_t)first_symbol_char->info.infoc < (uint8_t)second_symbol_char->info.infoc)
                symbol_result = '<';
            refal.previous_argument->tag = TAGO;
            refal.previous_argument->info.code = NULL;
            refal.previous_argument->info.infoc = symbol_result;
...
опишем на Си функцию, эквивалентную следующей рефал-функции:
    ...
    $FUNC Twokd
    {
        e.X '+' e.Y = <Function1 e.X> <Function2 e.Y>;
    }
...
Эта же функция описывается на Си:
    ...
    static void twokd_(void)
    {
        ...
        if (!extended_insert_from_free_memory(refal.previous_result, 6))
            return;
        T_LINKCB *k1 = refal.previous_result->next;
        T_LINKCB *symbol_label1 = k1->next;
        T_LINKCB *dot1 = symbol_label1->next;
        T_LINKCB *k2 = dot1->next;
        T_LINKCB *symbol_label2 = k2->next;
        T_LINKCB *dot2 = symbol_label2->next;
        transplantation(symbol_label1, refal.previous_argument, current_argument);
        transplantation(symbol_label2, current_argument, refal.next_argument);
        symbol_label1->tag = TAGF;
        symbol_label1->info.codef = &refalab_function1;
        symbol_label2->tag = TAGF;
        symbol_label2->info.codef = &refalab_function2;
        dot1->tag = TAGD;
        dot1->info.codep = k1;
        k1->tag = TAGK;
        k1->info.codep = dot2;
        dot2->tag = TAGD;
        dot2->info.codep = k2;
        k2->info.codep = refal.next_result->info.codep;
        refal.next_result->info.codep = dot1;
    }
...
- Минимальный размер списковой памяти в звеньях.
В начале работы РефалАБ-программы сразу захватывается под список
минимальное количество звеньев. Значение по умолчанию - 500.
- Максимальный размер списковой памяти в звеньях.
В процессе работы РефалАБ-программы не разрешается тратить
под список более чем максимальное количество звеньев, даже
если еще имеется свободная память в системе. Значение
по умолчанию - без ограничения.
- Количество звеньев, которое добавляется к списковой памяти
при каждом захвате дополнительной памяти. Значение
по умолчанию - 200.
- Количество звеньев, которое должно быть в списке свободных звеньев
после очередной успешной сборки мусора. Если в списке свободных звеньев меньше,
чем это количество, это означает, что к списковой памяти
следует добавить дополнительные звенья. Значение по умолчанию - 200.
...
- -minsz=число - изменить минимальный размер списковой памяти в звеньях.
Число может быть от 1 до 2147483647.
- -maxsz=число - изменить максимальный размер списковой памяти в звеньях.
Число может быть от 1 до 2147483647.
- -incrsz=число - изменить количество звеньев, которое добавляется
к списковой памяти при каждом захвате дополнительной памяти.
Число может быть от 1 до 2147483647.
- -freesz=число - изменить количество звеньев, которое должно быть
в списке свободных звеньев после очередной успешной сборки мусора.
Число может быть от 1 до 2147483647.

### 3.23. Аргументы командной строки
